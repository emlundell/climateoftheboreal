...
def surface(query, db):

    # Read sqlite query results into a pandas DataFrame
    home_path = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir))
    print("Connecting to {0}/data/ground_station/{1}".format(home_path, db))
    con = sqlite3.connect("{0}/data/ground_station/{1}".format(home_path, db))
    df = pd.read_sql_query(query, con)
    ...
    if 'DATE' in df.columns:
        df.index = df.DATE.apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d-%H').date())
        #del df["DATE"]
